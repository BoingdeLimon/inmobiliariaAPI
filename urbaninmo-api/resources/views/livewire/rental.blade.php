<div>
    <div class="grid grid-cols-1 gap-8 p-6">
        ...
        <div class="p-4 mt-4 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="p-6 max-w-lg mx-auto bg-white space-y-6">
                <h2 class="text-3xl font-bold text-gray-800">{{ 'Casa en Renta en ' . $rental['location'] }}</h2>
                <p class="text-lg text-gray-600">Bonita Casa en Fraccionamiento Privado.</p>
                <p class="text-3xl font-semibold text-[#3563E9]">{{ $rental['price'] }}/mes</p>
                <p class="text-md text-gray-500">{{ 'Ubicación: ' . $rental['location'] }}</p>
            ...
            <form wire:submit.prevent="submit"
                class="grid grid-cols-1 gap-4 bg-white shadow-xl rounded-lg p-6 transition-all duration-300 hover:shadow-2xl">
                <h3 class="text-lg font-bold">Contáctame</h3>
                ...
                <div>
                    <label class="block font-bold">Mensaje</label>
                    <textarea wire:model="message" placeholder="Escribe tu mensaje" rows="3" class="border rounded p-2 w-full"></textarea>
                    @error('message')
                        <span class="text-red-500">{{ $message }}</span>
                    @enderror
                </div>

                <button type="submit" class="bg-blue-600 text-white rounded p-2">Enviar</button>
                ...
                <label class="text-[10px] text-center">
