            ...
            <table class="min-w-full bg-white border border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                ...
                <tbody class="divide-y divide-gray-200 text-gray-700 dark:divide-gray-700 dark:text-gray-300">
                    @foreach ($users as $user)
                        <tr
                            class="border-b hover:bg-gray-100 dark:border-gray-700 dark:hover:bg-gray-700 justify-center align-middle">
                            <td class="px-4 py-2">
                                <img src="{{ $user->photo ? asset('storage/photos/' . $user->photo) : asset('img/default.jpg') }}"
                                    alt="{{ $user->name }}'s Photo"
                                    class="w-12 h-12 rounded-full object-cover border border-gray-300 dark:border-gray-600">
                            </td>
                            <td class="px-4 py-2 justify-center text-gray-900 dark:text-gray-300">{{ $user->name }}
                            </td>
                            ...
                            <td class="px-4 py-2 flex flex-row justify-evenly">
                                ...
                                <button wire:click="borrar({{ $user->id }})"
                                    wire:confirm.prompt="¿Esta seguro? Esta acción es irreversible\n\nEscribe BORRAR para continuar|BORRAR"
